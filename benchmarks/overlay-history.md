# Archived overlay measurements

Historical evidence for the retired overlay. Commands and behavior below describe those recorded revisions, not the current application. See [current testing guidance](../docs/testing.md).
...
Controlled visual-build idle sample: `baseline-visual-idle.json` records the requested two-minute run after the visual commit. It passed the unchanged budgets: 0.0256% of one CPU core, 19.08 MiB peak working set, 2.67 MiB private memory, 146.7 ms startup, and clean exit code 0. The report includes the exact executable/DLL hashes and raw samples. This is a single passing sample, not a multi-run median.
